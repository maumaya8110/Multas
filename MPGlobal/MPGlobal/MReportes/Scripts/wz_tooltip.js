...
var ShowHelp	  = false;		 //muestra o desactiva la ayuda OMJ
var ttAbove       = true;        // tooltip above mousepointer? Alternative: true
var ttBgColor     = "";
var ttBgImg       = "../js/helpframe.jpg";           // path to background image;
var ttBorderColor = "#003399";
var ttBorderWidth = 0;
var ttClickClose  = false;
var ttDelay       = 500;          // time span until tooltip shows up [milliseconds]
var ttFontColor   = "#000066";
var ttFontFace    = "tahoma";
var ttFontSize    = "9px";
...
var ttWidth       = 190;
var ttTitleBG	  = "../ToolTip/toolgrad.gif";
var ttHeig		  = 50;
var ttInfomation  = false;
...
function help_onclick()
{
	...
	if (ShowHelp==false)
	{
	    window.document.all["imghelp"].src = "/Imagenes/tabhelpon.jpg";
	    ShowHelp = true;
	    //window.document.all["helptab"].setAttribute("class", "tabhelpon");
	}
	else
	{
	    window.document.all["imghelp"].src = "/Imagenes/tabhelp.jpg";
	    ShowHelp = false;
	    //window.document.all["helptab"].setAttribute("class", "tabhelp");
	}
}
...
function tt_Htm(tt, t_id, txt)
{
	...
	t_bgimgT  = (typeof tt.T_TITLEBG != tt_u)? tt.T_TITLEBG  : ttTitleBG;
	var t_height=(typeof tt.T_HEIGHT != tt_u)? tt.T_HEIGHT  : ttHeig;
	...
	if (tt.T_DESCPANTALLA==true)
	{
		txt="<TABLE width=190 border=0 ><TR> <TD height=22></TD> <TD></TD> </TR> <TR> <TD height=50></TD> <TD valign=top class=ms-descriptiontextToolTip>" + txt + "</TD> </TR> </TABLE>" ;
	}
	else
	{
		var hImg="0";
		...
		txt="<TABLE width=180 border=0 bgColor=" + t_bgc + " ><TR> <TD height=" + hHeadImg + "></TD> <TD></TD> </TR> <TR> <TD height=" + hImg + "></TD> <TD valign=top class=ms-descriptiontextToolTip>" + txt + "</TD> </TR> </TABLE>" ;
	}
	if(tt_n4 && (t_fsz == "10px" || t_fsz == "11px")) t_fsz = "12px";
	var t_optx = (tt_n4? '' : tt_n6? ('-moz-opacity:'+(t_opa/100.0)) : tt_ie? ('filter:Alpha(opacity='+t_opa+')') : ('opacity:'+(t_opa/100.0))) + ';';
	var t_y = '<div id="'+t_id+'" style="position:absolute;z-index:1010;';
	t_y += 'left:0px;top:0px;width:'+(t_w+t_shw)+'px; visibility:'+(tt_n4? 'hide' : 'hidden')+';'+t_optx+'">' +
		'<table border="0" cellpadding="0" cellspacing="0"'+' width="'+t_w+'">';
	if(t_tit)
	{
		t_y += '<tr><td style="padding-left:15px;padding-right:3px;" align="'+t_algn+'"><font color="'+t_titc+'" face="'+t_ff+'" ' +
			'style="color:'+t_titc+';font-family:'+t_ff+';font-size:'+t_fsz+';"><b>' +
			(tt_n4? '&nbsp;' : '')+t_tit+'</b></font></td></tr>';
	}
	t_y += '<tr><td><table border="0" cellpadding="'+t_padd+'" cellspacing="'+t_bw+'" width="100%" height="'+t_height+'"px >' +
		'<tr><td'+ (t_bgimg? ' background="'+t_bgimg+'"' : '')+' vAlign="top" style="text-align:'+t_algn+';';
...
function tt_Show(t_e, t_id, t_sup, t_clk, t_delay, t_fix, t_left, t_offx, t_offy, t_static, t_sticky, t_temp, inf)
{
//	if (ShowHelp==true && inf!=true)
//		debugger;
    if (ShowHelp == false && inf != true)
        if (window.parent.ShowHelp != true) {
        return; }
		...
		{--j;
			if(typeof (t_tj = tags[j]).onmouseover == "function" && t_tj.onmouseover.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute("onmouseover")) && over.indexOf(esc) != -1)
			{
				...
				t_tj.onmouseover = new Function('e',
					...
					((typeof t_tj.T_INFORMATION != tt_u)? t_tj.T_INFORMATION : ttInfomation)+
					');'
				);
